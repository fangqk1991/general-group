...
(
    ...
    group_id    CHAR(32)        NOT NULL COMMENT '组 ID，具备唯一性',
    app         VARCHAR(31)     NOT NULL DEFAULT '' COMMENT '应用标识符',
...
(
    _rid        BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    group_id    CHAR(32)        NOT NULL COMMENT '组 ID，SQL 外键 -> common_group.group_id',
    FOREIGN KEY (group_id) REFERENCES common_group (group_id) ON DELETE RESTRICT,
    member      VARCHAR(127)    NOT NULL COMMENT '用户唯一标识；(group_id, member) 具备唯一性',
...
(
    ...
    group_id    CHAR(32)        NOT NULL COMMENT '组 ID，SQL 外键 -> common_group.group_id',
    FOREIGN KEY (group_id) REFERENCES common_group (group_id) ON DELETE RESTRICT,
    scope       VARCHAR(63)     NOT NULL COMMENT '范围描述项 | *',
    permission  VARCHAR(127)    NOT NULL DEFAULT '' COMMENT '权限描述项 GeneralPermission | *',
    create_time TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间: ISO8601 字符串',
    update_time TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间: ISO8601 字符串',
